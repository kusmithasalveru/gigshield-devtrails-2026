...
import { MapPin, ShieldCheck, Bolt, Loader, CreditCard, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export default function PayoutHistory() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const fallback = {
        payout_id: `sim-${Date.now()}`,
        amount: 140,
        status: 'completed',
        fraud_decision: 'auto_approve',
        fraud_score: 0.18,
        razorpay_ref: `sim-upi-${Date.now()}`,
      };
      setResp(fallback);
      const penalty = getTrustPenalty({ decision: fallback?.fraud_decision, anomalyScore: fallback?.fraud_score });
      if (penalty > 0) {
        updateUser({ trustScore: applyTrustPenalty(user?.trustScore, penalty) });
      }
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{t('payouts.eventDetails')}</p>
        </div>
        <span
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 p-5 backdrop-blur-xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <p className="font-semibold">{t('payouts.title')}</p>
        </div>
      ...
      {resp && (
        <div className="space-y-3">
          <motion.div
            ...
            className={`rounded-2xl p-4 border ${tone}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span className="text-[11px] font-semibold px-2 py-1 rounded-full bg-primary-50 text-primary-700 border border-primary-100">
                UPI ref: {resp.razorpay_ref || 'simulated'}
              </span>
            </div>
          </div>
        </div>
